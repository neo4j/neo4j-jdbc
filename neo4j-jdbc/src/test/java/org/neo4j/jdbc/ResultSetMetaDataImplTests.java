...
class ResultSetMetaDataImplTests {
	...
	// try (var resultSet =
	// setupWithValues(Collections.singletonList(Values.value("String")))) {
	// var rsMetadata = resultSet.getMetaData();
	...
	//
	// @Test
	// void testResultSetMetadataReturnsNullForTypeIfTheFirstRecordIsNull() throws
	// SQLException {
	...
	// @Test
	// void
	// testResultSetMetadataReturnsTheTypeOfTheFirstRecordIfTheFirstRecordTypeIsDifferent()
	// throws SQLException {
	// // This is a side effect of having the type be derived from first record. If we
	// // decide to change this
	// // we should update this test.
	//
	// try (var resultSet = setupWithValues(List.of(Values.value(1),
	// Values.value("String")))) {
	// var rsMetadata = resultSet.getMetaData();
	// Assertions.assertThat(rsMetadata.getColumnType(1)).isEqualTo(Neo4jConversions.toSqlType(Type.INTEGER));
	// }
