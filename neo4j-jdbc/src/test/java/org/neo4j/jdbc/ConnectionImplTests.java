...
class ConnectionImplTests {
	...
	// given(translator.translate(eq(sql),
	// any(DatabaseMetaData.class))).willReturn(expectedNativeSql);
	// var connection = new ConnectionImpl(mock(BoltConnection.class), () ->
	// List.of(translator), false, true, false,
	...
	// @ParameterizedTest
	// @ValueSource(booleans = { true, false })
	// void shouldCommitTransactionOnAutoCommitUpdate(boolean autoCommit) throws
	// SQLException {
	// var boltConnection = mock(BoltConnection.class);
	// var connection = makeConnection(boltConnection);
	// connection.setAutoCommit(!autoCommit);
	// var transactionType = connection.getAutoCommit() ? TransactionType.UNCONSTRAINED :
	// TransactionType.DEFAULT;
	// given(boltConnection.beginTransaction(eq(Collections.emptySet()), any(),
	// eq(AccessMode.WRITE),
	// eq(transactionType), eq(false)))
	...
	// .beginTransaction(eq(Collections.emptySet()), any(), eq(AccessMode.WRITE),
	// eq(transactionType), eq(false));
	...
	// @ParameterizedTest
	// @ValueSource(booleans = { true, false })
	// void shouldNotCommitTransactionOnSameAutoCommit(boolean autoCommit) throws
	// SQLException {
	// var boltConnection = mock(BoltConnection.class);
	// var connection = makeConnection(boltConnection);
	// connection.setAutoCommit(autoCommit);
	// var transactionType = connection.getAutoCommit() ? TransactionType.UNCONSTRAINED :
	// TransactionType.DEFAULT;
	// given(boltConnection.beginTransaction(Collections.emptySet(), Map.of(),
	// AccessMode.WRITE, transactionType,
	// false))
	// .willReturn(CompletableFuture.completedStage(null));
	// var transaction = connection.getTransaction(Map.of());
	//
	...
	// @ParameterizedTest
	// @ValueSource(booleans = { true, false })
	// void shouldThrowOnManagingAutoCommitTransaction(boolean rollback) throws
	// SQLException {
	// var boltConnection = mock(BoltConnection.class);
	// var connection = makeConnection(boltConnection);
	// given(boltConnection.beginTransaction(Collections.emptySet(), Map.of(),
	...
	// var transaction = connection.getTransaction(Map.of());
	//
	// ConnectionMethodRunner methodRunner = rollback ? Connection::rollback :
	// Connection::commit;
	//
	// assertThatThrownBy(() ->
	// methodRunner.run(connection)).isExactlyInstanceOf(SQLException.class);
	//
	// assertThat(Neo4jTransaction.State.NEW.equals(transaction.getState())).isTrue();
	// then(boltConnection).should()
	// .beginTransaction(eq(Collections.emptySet()), any(), eq(AccessMode.WRITE),
	// eq(TransactionType.UNCONSTRAINED), eq(false));
	// then(boltConnection).shouldHaveNoMoreInteractions();
	// }
	//
	// @ParameterizedTest
	...
	// given(boltConnection.beginTransaction(eq(Collections.emptySet()), any(),
	// eq(AccessMode.WRITE),
	// eq(TransactionType.DEFAULT), eq(false)))
	// .willReturn(CompletableFuture.completedStage(null));
	// if (rollback) {
	...
	// .equals(rollback ? Neo4jTransaction.State.ROLLEDBACK :
	// Neo4jTransaction.State.COMMITTED)).isTrue();
	// then(boltConnection).should()
	// .beginTransaction(eq(Collections.emptySet()), any(), eq(AccessMode.WRITE),
	// eq(TransactionType.DEFAULT),
	// eq(false));
	...
	// then(boltConnection).should()
	// .beginTransaction(eq(Collections.emptySet()), any(), eq(accessMode),
	// eq(TransactionType.UNCONSTRAINED),
	// eq(false));
	// }
	//
	// @Test
	...
	//
	// assertThatThrownBy(() ->
	// connection.setReadOnly(true)).isExactlyInstanceOf(SQLException.class);
	// }
	//
	// @Test
	...
	//
	// assertThatExceptionOfType(SQLException.class).isThrownBy(() ->
	// connection.setCatalog("someCatalog"))
	...
	//
	// @ParameterizedTest
	// @ValueSource(ints = { ResultSet.TYPE_SCROLL_INSENSITIVE,
	// ResultSet.TYPE_SCROLL_SENSITIVE })
	// void shouldThrowOnPreparingStatementWithUnsupportedResultSetType(int type) {
	// var connection = makeConnection(mock(BoltConnection.class));
	//
	// assertThatThrownBy(() -> connection.prepareStatement("sql", type,
	// ResultSet.CONCUR_READ_ONLY))
	...
	//
	// assertThatThrownBy(() ->
	// connection.isValid(-1)).isExactlyInstanceOf(SQLException.class);
	// }
	//
	// @ParameterizedTest
	// @MethodSource("getSendResetOnValidatingWithoutActiveTransactionArgs")
	// void shouldSendResetOnValidatingWithoutActiveTransaction(boolean
	// setupClosedTransaction, boolean expectedValid)
	// throws SQLException {
	// var boltConnection = mock(BoltConnection.class);
	// var connection = makeConnection(boltConnection);
	...
	// transaction.commit();
	// }
	// given(boltConnection.reset(true)).willReturn(expectedValid ?
	...
	// @ParameterizedTest
	// @ValueSource(booleans = { true, false })
	// void shouldAddWarningOnUnknownClientInfoProperty(boolean setupExistingWarning)
	// throws Exception {
	// var connection = makeConnection(mock(BoltConnection.class));
	...
	// assertThat((Object)
	// warning.getCause()).isExactlyInstanceOf(SQLClientInfoException.class);
	...
	// @ParameterizedTest
	// @ValueSource(booleans = { true, false })
	// void shouldAddWarningOnUnknownClientInfoProperties(boolean setupExistingWarning)
	// throws Exception {
	// var connection = makeConnection(mock(BoltConnection.class));
	// if (setupExistingWarning) {
	// var field = ConnectionImpl.class.getDeclaredField("warnings");
	...
	// then(boltConnection).should().beginTransaction(any(), any(), any(), any(),
	// anyBoolean());
	// }
	// then(boltConnection).should().close();
	// then(boltConnection).shouldHaveNoMoreInteractions();
	...
	// void shouldThrowWhenClosed(ConnectionMethodRunner consumer, Class<? extends
	...
	// assertThat(connection.isClosed()).isTrue();
	// assertThatThrownBy(() ->
	// consumer.run(connection)).isExactlyInstanceOf(exceptionType);
	// }
	//
	// static Stream<Arguments> getThrowingMethodExecutorsWhenClosed() {
	// return Stream.of(Arguments.of((ConnectionMethodRunner) Connection::createStatement,
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.prepareStatement("ignored"),
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.prepareCall("ignored"),
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.nativeSQL("ignored"),
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.setAutoCommit(false),
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::getAutoCommit,
	// SQLException.class),
	...
	// Arguments.of((ConnectionMethodRunner) connection -> connection.setReadOnly(false),
	// SQLException.class),
	...
	// Arguments.of((ConnectionMethodRunner) Connection::getTransactionIsolation,
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::getWarnings, SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::clearWarnings,
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.prepareStatement("ignored",
	// ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY), SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::getHoldability,
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.setClientInfo("ignored", "ignored"),
	// SQLClientInfoException.class),
	// Arguments.of((ConnectionMethodRunner) connection -> connection.setClientInfo(new
	// Properties()),
	// SQLClientInfoException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.getClientInfo("ignored"),
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::getClientInfo,
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.setSchema("ignored"),
	// SQLException.class),
	...
	// void shouldThrowUnsupported(ConnectionMethodRunner consumer, Class<? extends
	// SQLException> exceptionType) {
	// var connection = makeConnection(mock(BoltConnection.class));
	// assertThatThrownBy(() ->
	// consumer.run(connection)).isExactlyInstanceOf(exceptionType);
	...
	// SQLException.class),
	// Arguments.of((ConnectionMethodRunner) connection -> connection.prepareCall("RETURN
	// pi()",
	// ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE), SQLException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::getTypeMap,
	...
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::setSavepoint,
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.setSavepoint("ignored"),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.rollback(mock(Savepoint.class)),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.releaseSavepoint(mock(Savepoint.class)),
	// SQLFeatureNotSupportedException.class),
	// Arguments
	// .of((ConnectionMethodRunner) connection ->
	...
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.prepareStatement("ignored", new int[0]),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of(
	// (ConnectionMethodRunner) connection -> connection.prepareStatement("ignored", new
	// String[0]),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::createClob,
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::createBlob,
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::createNClob,
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) Connection::createSQLXML,
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.createArrayOf("ignored", new String[0]),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	// connection.createStruct("ignored", new Object[0]),
	// SQLFeatureNotSupportedException.class),
	// Arguments.of((ConnectionMethodRunner) connection ->
	...
	// @ParameterizedTest
	// @MethodSource("getUnwrapArgs")
	// void shouldHandleIsWrapperFor(Class<?> cls, boolean shouldUnwrap) throws
	// SQLException {
	// // given
	// var connection = makeConnection(mock(BoltConnection.class));
	//
	...
	// return new ConnectionImpl(boltConnection, List::of, false, false, true, false, new
	// VoidBookmarkManagerImpl(),
	// Map.of());
	//
	// }
	//
	// private static Stream<Arguments> getUnwrapArgs() {
	// return Stream.of(Arguments.of(ConnectionImpl.class, true),
	// Arguments.of(Connection.class, true),
	...
	//
	// @Override
	// public String translate(String statement, DatabaseMetaData
	// optionalDatabaseMetaData) {
	...
	// translatorChain.apply("x")).withMessage("x != s1");
	// assertThat(warnings).isEmpty();
	// }
	//
	// @Test
	...
	// List.of(new StaticTranslator("whatever"), new StaticTranslator("s2")), null,
	// warnings::add);
	// assertThat(translatorChain.apply("s2")).isEqualTo("s2_translated");
	// assertThat(warnings).hasSize(1)
	// .extracting(SQLWarning::getMessage)
	// .first()
	// .isEqualTo("Translator org.neo4j.jdbc.ConnectionImplTests$StaticTranslator failed
	// to translate `s2`");
	// }
	//
	// @Test
	...
	// var warnings = new ArrayList<SQLWarning>();
	// var translatorChain = new TranslatorChain(List.of(new StaticTranslator("s1"),
	// new StaticTranslator("whatever"), new StaticTranslator("s1_translated")), null,
	// warnings::add);
	...
	// .first()
	// .isEqualTo(
	// "Translator org.neo4j.jdbc.ConnectionImplTests$StaticTranslator failed to translate
	// `s1_translated`");
	// }
	//
	// @Test
	...
	// var warnings = new ArrayList<SQLWarning>();
	// var translatorChain = new TranslatorChain(List.of(new StaticTranslator("s1"),
	// new StaticTranslator("s1_translated"), new
	// StaticTranslator("s1_translated_translated")), null,
	// warnings::add);
	...
	// var warnings = new ArrayList<SQLWarning>();
	// var translatorChain = new TranslatorChain(
	// List.of(new StaticTranslator("a"), new StaticTranslator("b"), new
	// StaticTranslator("c")), null,
	// warnings::add);
	...
	// var warnings = new ArrayList<SQLWarning>();
	// var translatorChain = new TranslatorChain(List.of(new StaticTranslator("s1"),
	// new StaticTranslator("s1_translated"), new StaticTranslator("c")), null,
	// warnings::add);
