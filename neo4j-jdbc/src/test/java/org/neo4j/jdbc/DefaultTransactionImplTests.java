...
class DefaultTransactionImplTests {
	...
	// static DefaultTransactionImpl.FatalExceptionHandler NOOP_HANDLER = (ignored1,
	// ignored2) -> {
	// };
	//
	...
	// this.transaction = new DefaultTransactionImpl(boltConnection, null, null,
	// NOOP_HANDLER,
	...
	// given(boltConnection.pull(runFuture,
	// fetchSize)).willReturn(CompletableFuture.completedFuture(pullResponse));
	//
	...
	// given(boltConnection.discard(-1,
	// !commit)).willReturn(CompletableFuture.completedFuture(discardResponse));
	// if (commit) {
	// given(boltConnection.commit()).willReturn(CompletableFuture.completedFuture(null));
	// }
	//
	// var response = this.transaction.runAndDiscard(query, parameters, fetchSize,
	// commit);
	//
	// assertThat(response).isNotNull();
	// assertThat(response).isEqualTo(discardResponse);
	// assertThat(this.transaction.getState())
	// .isEqualTo(commit ? Neo4jTransaction.State.COMMITTED :
	// Neo4jTransaction.State.READY);
	...
	// var runResponse = mock(RunResponse.class);
	// var pullResponse = mock(PullResponse.class);
	// given(boltConnection.pull(runResponse,
	// fetchSize)).willReturn(CompletableFuture.completedFuture(pullResponse));
	//
	...
	// var boltConnection = mock(BoltConnection.class);
	// given(boltConnection.beginTransaction(anySet(), anyMap(), any(AccessMode.class),
	...
	// this.transaction = new DefaultTransactionImpl(boltConnection, null, null,
	// NOOP_HANDLER,
	// CompletableFuture.completedStage(null), autoCommit, null,
	// Neo4jTransaction.State.READY);
	...
	//
	// assertThat(this.transaction.getState())
	// .isEqualTo(autoCommit ? Neo4jTransaction.State.FAILED :
	// Neo4jTransaction.State.OPEN_FAILED);
	// assertThat(this.transaction.isRunnable()).isEqualTo(false);
	...
	// this.transaction = new DefaultTransactionImpl(boltConnection, null, null,
	// NOOP_HANDLER,
	// CompletableFuture.completedStage(null), false, null,
	// Neo4jTransaction.State.OPEN_FAILED);
	//
	// this.transaction.rollback();
	//
	...
	// assertThat(this.transaction.isOpen()).isEqualTo(false);
	// then(boltConnection).should()
	// .beginTransaction(Set.of(), Map.of(), AccessMode.WRITE, TransactionType.DEFAULT,
	// false);
	// then(boltConnection).shouldHaveNoMoreInteractions();
	// }
	//
	...
	// void shouldThrowInInvalidState(boolean autocommit, Neo4jTransaction.State state,
	// TransactionMethodRunner runner) {
	// var boltConnection = mockBoltConnection();
	// this.transaction = new DefaultTransactionImpl(boltConnection, null, null,
	// NOOP_HANDLER,
	...
	//
	// assertThatThrownBy(() ->
	// runner.run(this.transaction)).isExactlyInstanceOf(SQLException.class);
	...
	// (TransactionMethodRunner) transaction -> transaction.pull(mock(RunResponse.class),
	// -1)),
	...
	// -1)),
	// Arguments.of(true, Neo4jTransaction.State.FAILED,
	// (TransactionMethodRunner) transaction -> transaction.runAndPull("query",
	...
	// (TransactionMethodRunner) transaction -> transaction.runAndDiscard("query",
	// Collections.emptyMap(), -1, false)),
	// Arguments.of(true, Neo4jTransaction.State.FAILED, (TransactionMethodRunner)
	// Neo4jTransaction::commit),
	// Arguments.of(false, Neo4jTransaction.State.FAILED, (TransactionMethodRunner)
	// Neo4jTransaction::commit),
	// Arguments.of(true, Neo4jTransaction.State.COMMITTED,
	...
	// this.transaction = new DefaultTransactionImpl(mockBoltConnection(), null, null,
	// NOOP_HANDLER,
	...
	// var boltConnection = mock(BoltConnection.class);
	// var fatalExceptionHandler =
	// mock(DefaultTransactionImpl.FatalExceptionHandler.class);
	// this.transaction = new DefaultTransactionImpl(boltConnection, null, null,
	...
	// fetchSize)).willReturn(CompletableFuture.completedFuture(pullResponse));
	//
	// assertThatThrownBy(() -> this.transaction.runAndPull(query, parameters, fetchSize,
	// 1))
	...
	// var discardResponse = mock(DiscardResponse.class);
	// given(boltConnection.discard(-1,
	// true)).willReturn(CompletableFuture.completedFuture(discardResponse));
	//
	// assertThatThrownBy(() -> this.transaction.runAndDiscard(query, parameters, 1,
	// false))
	// .isExactlyInstanceOf(SQLTimeoutException.class);
	// assertThat(this.transaction.getState())
	// .isEqualTo(autoCommit ? Neo4jTransaction.State.FAILED :
	...
	// assertThat(this.transaction.isRunnable()).isFalse();
	// assertThat(this.transaction.isOpen()).isEqualTo(false);
	// then(fatalExceptionHandler).should().handle(any(SQLException.class),
	...
	// given(boltConnection.run(query, parameters,
	// false)).willReturn(CompletableFuture.completedFuture(runResponse0))
	...
	// }
	//
	// then(boltConnection).should().beginTransaction(anySet(), anyMap(), any(), any(),
	// anyBoolean());
	...
	// return Stream.of(
	// Arguments.of((TransactionMethodRunner) transaction ->
	// transaction.runAndPull("query",
	// Collections.emptyMap(), -1, 0)),
	// Arguments.of((TransactionMethodRunner) transaction ->
	// transaction.runAndDiscard("query",
	// Collections.emptyMap(), -1, true)),
	// Arguments.of((TransactionMethodRunner) transaction ->
	// transaction.runAndDiscard("query",
	// Collections.emptyMap(), -1, false)),
