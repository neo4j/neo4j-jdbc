...
import org.neo4j.driver.jdbc.internal.bolt.exception.Uncoercible;
...
public interface Value extends MapAccessor, MapAccessorWithDefaultValue {
	 ...
	 * <p>
	 * For {@link TypeSystem#MAP() map} values, this will return the number of entries in
	 * the map.
	 * <p>
	 * For {@link TypeSystem#NODE() node} and {@link TypeSystem#RELATIONSHIP()}
	 * relationship} values, this will return the number of properties.
	 * <p>
	 * For {@link TypeSystem#PATH() path} values, this returns the length (number of
	 ...
	 * iterable of the keys in the map, this applies to {@link TypeSystem#MAP() map},
	 * {@link #asNode() node} and {@link TypeSystem#RELATIONSHIP()} relationship} values.
	 ...
	 * @throws BoltException if record has not been initialized
	 */
	Value get(int index);
	 ...
	 * </ul>
	 * <p>
	 * Note that the types in {@link TypeSystem} refers to the Neo4j type system where
	 * {@link TypeSystem#INTEGER()} and {@link TypeSystem#FLOAT()} are both 64-bit
	 * precision. This is why these types return java {@link Long} and {@link Double},
	 * respectively.
	 ...
	 * mapping he original map values, if possible
	 * @see Values for a long list of built-in conversion functions
	 */
