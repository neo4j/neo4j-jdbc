...
public sealed interface Vector extends AsValue {
	...
	static Vector int64(long[] elements) {
		assertSize(Objects.requireNonNull(elements, "Vector elements must not be literal null").length);
		return new ArrayBasedVectors.Int64VectorImpl(ElementType.INTEGER, elements.length, elements);
	}
	...
	static Vector float64(double[] elements) {
		assertSize(Objects.requireNonNull(elements, "Vector elements must not be literal null").length);
		return new ArrayBasedVectors.Float64VectorImpl(ElementType.FLOAT, elements.length, elements);
	}
