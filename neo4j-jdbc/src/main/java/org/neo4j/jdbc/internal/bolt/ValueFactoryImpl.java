...
enum ValueFactoryImpl implements ValueFactory {
	...
	public Value vector(Class<?> elementType, Object elements) {
		...
		if (elementType == byte.class) {
			vector = Vector.int8((byte[]) elements);
		}
		else if (elementType == short.class) {
			vector = Vector.int16((short[]) elements);
		}
		else if (elementType == int.class) {
			vector = Vector.int32((int[]) elements);
		}
		else if (elementType == long.class) {
			vector = Vector.int64((long[]) elements);
		}
		else if (elementType == float.class) {
			vector = Vector.float32((float[]) elements);
		}
		else if (elementType == double.class) {
			vector = Vector.float64((double[]) elements);
		}
		else {
