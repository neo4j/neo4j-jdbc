...
* through the `Bolt` protocol (3.0.x to 3.3.0) using `jdbc:neo4j:bolt://<host>:<port>/`
* through the `Bolt+Routing` protocol (3.3.1) using `jdbc:neo4j:bolt+routing://<host>:<port>/`
...
** `jdbc:neo4j:bolt+routing://<host>:<port>/` (e.g. `jdbc:neo4j:bolt://localhost?region=europe&country=it`)
