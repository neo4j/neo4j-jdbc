...
* through the Bolt protocol (3.0.x to 3.2.x) using `jdbc:neo4j:bolt://<host>:<port>/`
...
* Install a Neo4j 3.2.x server and start it with the Bolt protocol enabled
...
[NOTE]
To run the performance test, you must have a Neo4j Server 3.2.x running with the Bolt protocol enabled on port 7687 (default)
