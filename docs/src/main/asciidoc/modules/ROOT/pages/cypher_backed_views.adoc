...
WARNING: Make sure you only load view definitions from trusted sources. As stated above, there's nothing that prevents a writing Cypher query being used as a view definition, and you don't want to have them injected into your application from an unknown or untrusted source. Using `http` without encryption is only recommended for testing.
