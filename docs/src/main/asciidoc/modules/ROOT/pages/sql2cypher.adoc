...
Named groups `start`, `reltype` and `end` are supported too. `null` or a blank pattern disables automatic inference.
...
The matching has been based on the virtual table. If both nodes had had a `name` attribute, the property would have been used for the start node only.
Columns that match neither properties of the nodes nor the relationship, will be added to the relationship.

WARNING: If the schema spots the same property name on nodes and relationships, the nodes are prioritized. If the same property appears on both sides of the relationship, it will be only used on the first appearance (the start node). If this does not work for you, you must qualify the target properties.
