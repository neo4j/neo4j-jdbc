...
[source, text]
.Canonical JDBC URL format for Neo4j JDBC Driver
----
...
Last but not least, you can add `http` as sub-protocol when using the Neo4j-JDBC driver against a Neo4j instance from 2025.06 on with the Http Query-API enabled:
...
Usually, Neo4j provides the HTTP port on 7474 or 7473. When you don't specify the port, the Neo4j JDBC Driver defaults to 80 and 443 for http and https respectively.
...
|`authn.supplier`
|`String`
|The name of an authentication supplier, can be used to plugin various SSO systems such as Keycloak, with automatic token refresh. See below xref:authentication[] for more information.
|`null`

|`tryTcpFastOpen`
|`boolean`
a|Setting this value to `true` makes the driver attempt to use https://en.wikipedia.org/wiki/TCP_Fast_Open[TCP Fast Open]. For this to work the following additional requirements must be fulfilled:

* The system the driver is running on supports and has TCP Fast Open enabled.
* The DBMS to which the driver should connect must also support and enable TCP Fast Open
* One of the following additional runtime dependencies (depending on your operating system), must be present on the class- or module path:
...
TIP: Providing that a valid Netty dependency for transport is added, it will be discovered and used instead of the default NIO transport. Those are operating system specific and valid options are `netty-transport-native-epoll`, `netty-transport-native-kqueue` and on Netty 4.2 or later, `netty-transport-native-io_uring`.
...
----
include::{examplesDir}/java/Configuration.java[tag=cdm]
----
...
----
include::{examplesDir}/java/Configuration.java[tag=ddm]
----
...
----
include::{examplesDir}/java/TransactionMetadata.java[tag=example]
----
...
[source,properties]
.Logging configuration for the Neo4j JDBC Driver that logs all information
----
.level = INFO
...
public class TracingExampleSetup {
    ...
    Connection connection() {
        ...
        connection.setTracer(Neo4jTracingBridge.to(tracer)); // <.>
    }
}
----
...
TIP: In case you neither want to work with a data source nor individual connections, use `Neo4jDriver.registerTracer(Neo4jTracingBridge.to(tracer))` to register the tracer with all Neo4j drivers that are registered with the JDBC `DriverManager`.
...
[source,java, tabsize=4, indent=0]
.Configure tracing for all drivers registered with `DriverManager`
----
import io.micrometer.tracing.Tracer;
import org.neo4j.jdbc.tracing.micrometer.Neo4jTracingBridge;
...
An `Authentication` instances is an abstract representation of an arbitrary authentication to be presented to the Neo4j server and can contain a principal and its credentials (username and password), a token or other ways of confirming an identity.

The interface itself offers the following factories:

`Authentication#usernameAndPassword`:: Creates a simple username and password based authentication, which is what is used internally already with the default methods
`Authentication#none`:: Disables authentication
`Authentication#bearer`:: Creates a bearer-token based authentication

The latter allows to set an expiration date for the token.
If the token expires, that is used after the expiration date, the supplier is asked for a new one.
This allows configuring the Neo4j JDBC driver in such a way that it can grab tokens from an SSO provider, such as https://www.keycloak.org[Keycloak] or https://www.okta.com[Okta], and authenticate itself with those tokens against Neo4j and automatically refresh those tokens when they expire.
...
* The convenient builder methods present as static methods on `Neo4jDriver` allowing to configure the driver from the environment (see xref:usage.adoc#connect-dotenv[Getting a connection via environment variables]) allow to specify the supplier, too

The precedence is as follows:

. If there's a supplier passed to `connect` it has precedence over the drivers locally configured supplier respectively the global one and any properties
. If there's a property named `authn.supplier` in the JDBC properties and an authentication supplier factory with that name is available, a supplier will be sourced from that factory and used
...
The driver will only call the supplier when no token has been retrieved yet or when the first token expires.
Any authentication supplier implementation can safely assume that the second call for a token means the first token is expired, so that the supplier does not need to manage state.

NOTE: The underlying bolt protocol and the Neo4j Server might not work reliable with very short-lived authentication tokens (i.e. with a lifespan under 5 seconds). This restriction might be lifted in the future, but as of release 6.6.0 you might want to refrain from very short-lived tokens.

=== Keycloak example

The following classes shows an example based on Keycloaks `AuthzClient`.
...
Again, for using the above supplier add the following dependency to your class- or module-path and don't copy the code, which is only displayed here as an example how you would write your own authentication supplier:
...
The easiest way to use the supplier now is via the build-in loader, so that you can stick purely to the JDBC api:

[source,java]
.Using a custom authentication supplier via the `authn.supplier` config option:
----
@Test
...
----
<.> Indicate that you want to use the Keycloak authentication supplier (named `kc` in our distribution)
<.> This and the following properties will be passed to the authentication system so that it can create authentication with Keycloak
<.> This is a pure JDBC spec call
...
[source,java]
.Registering an instance of the authentication supplier globally
----
import java.sql.Connection;
import java.sql.DriverManager;
import java.util.function.Supplier;
...
The advantage here is that it makes sure that all applications that use the Neo4j JDBC Driver in a JDBC compliant way over the `DriverManager` will automatically use the authentication supplier.
...
[source,java]
.Creating an explicit instance of `Neo4jDriver` so that the `connect` overload taking in an authentication supplier is visible
----
import java.sql.Connection;
import java.sql.DriverManager;
...
TIP: If you need to against Keycloak, the Neo4j JDBC driver does ship everything you need, and you are good to go.
Just include the Maven module and apply the recommended way of configuring the supplier via JDBC properties.
Write a custom authentication supplier only for other authentication needs.
