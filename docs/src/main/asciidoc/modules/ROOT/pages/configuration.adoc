...
An authentication supplier is in Java terms a `Supplier<org.neo4j.jdbc.Authentication>`.
...
1. If there's supplier passed to `connect` it has precedence over the drivers locally configured supplier respectively the global one and any properties
2. The supplier configured per driver or the global one has precedence over any properties
3. Properties will be used as per JDBC standard when there is no explicit or implicit authentication supplier
...
.`AuthzAuthenticationSupplier`, an example of using Keycloak with the Neo4j JDBC driver
----
...
final class AuthzAuthenticationSupplier implements Supplier<Authentication> {
...
To use this supplier, you can either register it for the global driver like this:
...
class Example {
	...
	public static void main(String...a) throws Exception {
		Supplier<Authentication> authenticationSupplier = new AuthzAuthenticationSupplier(new Configuration());
...
The above setting is most likely the easiest way for integrations with frameworks like Spring Boot, in which you could configure the global supplier through an `org.springframework.beans.factory.InitializingBean`.
