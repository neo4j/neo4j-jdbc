...
class SparkSubqueryCleaningTranslatorTests {
	...
	static Stream<Arguments> shouldDetectParsableSQL() {
		return Stream.of(Arguments.of(
				"MATCH (m:Movie)<-[:ACTED_IN]-(p:Person) RETURN m.title AS title, collect(p.name) AS actors ORDER BY m.title",
				false), Arguments.of("INSERT INTO Movie(title, year) VALUES (?, ?)", false),
				Arguments.of("SELECT * FROM Movie", true));
	}

	@ParameterizedTest
	@MethodSource
	void shouldDetectParsableSQL(String statement, boolean expectation) {

		var translator = new SparkSubqueryCleaningTranslator(SqlToCypherConfig.builder().build());
		assertThat(translator.canParseAsSelect(statement)).isEqualTo(expectation);
	}
